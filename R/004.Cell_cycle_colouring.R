...
#Remove genes that are not expressed in both cells
...
k_seu <- FindVariableFeatures(k_seu, selection.method = "vst",nfeatures = 10000)
...
png(file='cell-cycle_phases.png',width = 700,height=700, res=100)
par(mfrow=c(1,2))
k <-plot(phases_k, col=c('coral','chartreuse3','cyan3'),main='KELLY cells phases',ylim=c(0,500),ylab='Number of cells'
         ,cex.lab=1.5,cex.main=1.5,font=2)
text(k,table(phases_k)+50,labels =paste0(round(perc_k),rep('%',3)),font=2)
b <- plot(phases_b, col=c('coral','chartreuse3','cyan3'),main='BE(2)-C cells phases',ylim=c(0,500),ylab='Number of cells'
          ,cex.lab=1.5,cex.main=1.5,font=2)
text(b,table(phases_b)+50,labels =paste0(round(perc_b),rep('%',3)),font=2)
dev.off()
